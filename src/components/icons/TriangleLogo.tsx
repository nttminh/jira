...
const TriangleLogo = ({ color = 'gray', ...props }: Props) => {
	return (
		<span
			aria-label="Atlassian"
			role="img"
			// style="--logo-color:#2684FF; --logo-fill:#0052CC;"
			style={{
				color: color === 'blue' ? '#2684FF' : '#5E6C84',
				fill: color === 'blue' ? '#0052CC' : '#42526E',
			}}
			{...props}
		>
			<svg
			...
			>
				<defs>
					...
					>
						<stop
							stopColor={color === 'blue' ? '#2684FF' : '#344563'}
							offset="0%"
						></stop>
						<stop
							stopColor={color === 'blue' ? '#2684FF' : '#7A869A'}
							offset="0.9228"
						></stop>
					</linearGradient>
				</defs>
				<path
					fill={color === 'blue' ? '#2684FF' : '#344563'}
